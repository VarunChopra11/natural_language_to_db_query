...
async def portfolio_summary(user: dict = Depends(get_current_client)):  # Updated dependency
    return await get_client_portfolio_summary(user["id"])

@router.get("/analytics/token-distribution")
async def token_distribution(user: dict = Depends(get_current_client)):  # Updated dependency
    return await get_client_token_distribution(user["id"])

@router.get("/analytics/app-summary")
async def app_summary(user: dict = Depends(get_current_client)):  # Updated dependency
    return await get_client_app_summary(user["id"])

@router.get("/analytics/nft-summary")
async def nft_summary(user: dict = Depends(get_current_client)):  # Updated dependency
    return await get_client_nft_summary(user["id"])

@router.get("/analytics/transaction-activity")
async def transaction_activity(
    user: dict = Depends(get_current_client),  # Updated dependency
    ...
    return await get_client_transaction_activity(user["id"], range)  # Use user["id"]